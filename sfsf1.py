...
class RUN:
    ...
    def get_SignTaskList(self, END=False):
        ...
        if response.get('success') == True and response.get('obj') != []:
            ...
            for task in response["obj"]["taskTitleLevels"]:
                ...
                status = task["status"]
                skip_title = ['用行业模板寄件下单', '去新增一个收件偏好', '参与积分活动', '用积分兑任意礼品']
                # 任务已完成
                if status == 3:
                    continue
                # 跳过的任务
                if self.title in skip_title:
                    print(f'跳过任务[{self.title}]')
                    continue
                else:
    ...
    def doTask(self):
        ...
        response = self.do_request(url, data=json_data)
        if response.get('success') == True:
            print(f'完成任务[{self.title}]成功')
        else:
            print(f'完成任务[{self.title}]失败')
    ...
    def receiveTask(self):
        ...
        response = self.do_request(url, data=json_data)
        if response.get('success') == True:
            point = response[ 'obj' ][ 'point' ]
            print(f'完成任务[{self.title}]奖励：{point}积分')
        else:
            print(f'完成任务[{self.title}]失败')
    ...
    def do_honeyTask(self):
        ...
        response = self.do_request(url, data=json_data)
        if response.get('success') == True:
            print(f'[{self.taskType}]任务已完成')
        else:
            print(f'[{self.taskType}]任务完成失败')
