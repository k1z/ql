...
class Config:
    ...
    def __post_init__(self):
        if self.SKIP_TASKS is None:
            ...
            self.SKIP_TASKS = ['用行业模板寄件下单', '用积分兑任意礼品', '参与积分活动', '每月累计寄件', '完成每月任务',
                               '去使用AI寄件']
            ...
            try:
                ...
                if success:
                    ...
                    if retry_count < MAX_PROXY_RETRIES-1:
                        print(
                            f'账号{self.account_index} 登录失败，尝试重新获取代理 ({retry_count+1}/{MAX_PROXY_RETRIES})')
                        time.sleep(2)
            except Exception as e:
...
def main():
    ...
    print("-" * 80)
    print(
        f"{'汇总':<6} {'账号总数: '+str(len(all_results)):<15} {'今日总获得: '+str(total_earned):<15} {'':<15} {'成功: '+str(success_count):<10}")
    print("=" * 80)
